...
$get_user->bind_param('s', $_SERVER['REMOTE_ADDR']);
$get_user->execute();
...
curl_setopt_array($ch, array(
    CURLOPT_URL => 'http://check.getipintel.net/check.php?ip='. $_SERVER['REMOTE_ADDR'] .'&contact=energeticbark37@gmail.com&flags=f',
    CURLOPT_RETURNTRANSFER => true));
$is_proxy = curl_exec($ch);
curl_close($ch);

if ($is_proxy > 0.99) {
    exit('You can\'t create an account when using a proxy. If you\'re trying to bypass a ban fuck off.');
}
...
if (empty($_SESSION['signed_in'])) {
	if($_SERVER['REQUEST_METHOD'] != 'POST'){
        ...
        <div class="hb-contents-wrapper"><div class="hb-container hb-l-inside">
            <h2>Sign Up</h2>
            <p>Create a User ID for Closedverse.</p>
        </div>
    ...
    <?php
    	...
    	if (isset($_POST['submit'])) {
    		...
    		if (!empty($errors)){
    			echo '<script type="text/javascript">alert("' . $errors . '");</script><META HTTP-EQUIV="refresh" content="0;URL=/signup">';
    		} else {
    			...
    			$get_user = $dbc->prepare('SELECT user_id, user_name FROM users WHERE user_name = ? LIMIT 1');
