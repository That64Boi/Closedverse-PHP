...
if ((isset($_GET['offset']) && is_numeric($_GET['offset'])) && isset($_GET['date'])) {
    ...
    if ($title_result->num_rows == 0) {
        ...
        <div class="tab3"><a id="posts-filter-anchor" href="/titles/'. $title['title_id'] .'"><span class="new-posts">All Posts</span></a>
        <a class="selected" href="/titles/'. $title['title_id'] .'/popular">Popular <span class="long">posts</span></a><a class="" href="/titles/'. $title['title_id'] .'/controversial">Controversial <span class="long">posts</span></a></div></div><div class="pager-button">';

        if (date("Y-m-d H:i:s", strtotime($date)) < date("Y-m-d")) {
            echo '<a href="/titles/'. $title['title_id'] .'/popular?date='. date("Y-m-d H:i:s", strtotime($date . '+1 day')) .'" class="button back-button symbol"><span class="symbol-label">←</span></a>';
        }
        
        echo '<a href="/titles/'. $title['title_id'] .'/popular?date='. $date .'" class="button selected">'. date("m/d/Y", strtotime($date)) .'</a><a href="/titles/'. $title['title_id'] .'/popular?date='. date("Y-m-d H:i:s", strtotime($date . '-1 day')) .'" class="button next-button symbol"><span class="symbol-label">→</span></a></div>';
