...
if ($user_result->num_rows == 0) {
    ...
    if (!((isset($_GET['offset']) && is_numeric($_GET['offset'])) && isset($_GET['dateTime']))) {
        $tabTitle = htmlspecialchars($user['nickname'], ENT_QUOTES) .'\'s Nahs - Cedar';
        ...
        while ($nahs = $nahs_result->fetch_array()) {
            if ($nahs['type'] == 0) {
                ...
                if ($replies_result->num_rows == 0) {
                    $delete_nah = $dbc->prepare('DELETE FROM nahs WHERE nah_post = ? AND type = ?');
                    $delete_nah->bind_param('ii', $nahs['nah_post'], $nahs['type']);
                    $delete_nah->execute();
                    continue;
                }
